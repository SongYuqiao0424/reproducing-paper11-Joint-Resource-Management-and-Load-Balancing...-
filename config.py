...
class Config:
    ...
    # 优化SCA算法：测试发现L_0过大导致功率分配会较为极端，经常将全部功率分配给1/2个波束,在激活的四个波束队列长度差异较大时尤为明显；
    ...
    # 更新信道增益h、g的计算后，对算法进行测试
    # 测试SCA算法过程中发现，由于当前实际传输量为P*100，与算法中代替函数R相差较多，算法得到的功率可能不足以解决队列需求，待实际传输量修正为SINR版后再测试
